  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="p-3.5 rounded-xl bg-[#230d02] border border-[rgba(240,190,100,0.06)] flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] text-[#9a7c60]">Web Push Subscribers</p>
        ...
        <div className="p-3.5 rounded-xl bg-[#230d02] border border-[rgba(240,190,100,0.06)] flex items-center gap-3">
          ...
          <div>
            <p className="text-[10px] text-[#9a7c60]">Mobile Push Subscribers</p>
      ...
      <div className="p-4 rounded-xl bg-[#230d02] border border-[rgba(240,190,100,0.06)]">
        <h3 className="text-xs font-semibold text-white tracking-wide mb-4 flex items-center gap-2">
          <Send className="w-3.5 h-3.5 text-[#E05A1A]" /> Send Daily Word
        </h3>
        {error && (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-[11px] font-medium text-[#9a7c60] mb-1.5">Type</label>
            <div className="flex gap-2">
              {['verse', 'prophetic_word'].map(t => (
                <button key={t} type="button" onClick={() => setType(t)}
                  className={`px-3 py-1.5 rounded-lg text-[11px] font-medium border transition-colors ${
                    type === t ? 'bg-[#E05A1A]/10 border-[#E05A1A]/30 text-[#E05A1A]' : 'bg-[#2f1206] border-[rgba(240,190,100,0.08)] text-[#9a7c60]'
                  }`}>
          ...
          <div>
            <label className="block text-[11px] font-medium text-[#9a7c60] mb-1.5">Title</label>
          ...
          <div>
            <label className="block text-[11px] font-medium text-[#9a7c60] mb-1.5">Content</label>
          ...
          <div>
            <label className="block text-[11px] font-medium text-[#9a7c60] mb-1.5">Reference (optional)</label>
      ...
      <div className="p-4 rounded-xl bg-[#230d02] border border-[rgba(240,190,100,0.06)]">
        ...
        {fetching ? (
          <div className="py-6 text-center text-[11px] text-[#9a7c60]">Loading...</div>
        ) : verses.length === 0 ? (
          <div className="py-6 text-center text-[11px] text-[#9a7c60]">No daily words sent yet</div>
        ) : (
          <div className="space-y-3">
            {verses.map(v => (
              <div key={v.id} className="p-3 rounded-lg bg-[rgba(240,190,100,0.02)] border border-[rgba(240,190,100,0.06)]">
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    {v.type === 'verse' ? <BookOpen className="w-3 h-3 text-[#E05A1A]" /> : <Sparkles className="w-3 h-3 text-[#8b7cf8]" />}
                    <span className="text-[11px] font-medium text-white">{v.title}</span>
                  </div>
                  <span className="text-[9px] text-[#9a7c60]">{v.created_at ? new Date(v.created_at).toLocaleDateString() : ''}</span>
                </div>
                <p className="text-[11px] text-[#9a7c60] leading-relaxed">{v.content}</p>
                {v.reference && <p className="text-[9px] text-[#E05A1A] mt-1">{v.reference}</p>}
              </div>
            ))}
          </div>
        )}
