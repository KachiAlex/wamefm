...
router.post('/', authenticateToken, requireRole('admin'), upload.single('audio'), async (req, res) => {
  try {
    ...
    await db.run(
      `INSERT INTO sermons (id, title, description, scripture_reference, speaker, series, audio_url, date, duration)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
      [id, title, description || null, scripture_reference || null, speaker || null, series || null, audioUrl, date, duration ? parseInt(duration, 10) : null]
    )
