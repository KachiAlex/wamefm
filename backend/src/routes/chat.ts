...
router.get('/broadcast/:broadcastId', async (req, res) => {
  try {
    ...
    const messages = await db.all(
      'SELECT * FROM chat_messages WHERE broadcast_id = $1 ORDER BY created_at ASC',
...
router.post('/broadcast/:broadcastId', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const id = uuidv4()
    await db.run(
      'INSERT INTO chat_messages (id, broadcast_id, user_id, user_name, message) VALUES ($1, $2, $3, $4, $5)',
      [id, req.params.broadcastId, req.user!.id, req.user!.name, message.trim()]
    )
