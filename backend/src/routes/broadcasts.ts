...
router.post('/', authenticateToken, requireRole('broadcaster', 'admin'), async (req: AuthenticatedRequest, res) => {
  try {
    ...
    const id = uuidv4()
    await db.run(
      `INSERT INTO broadcasts (id, title, description, scripture_reference, status, started_at, broadcaster_id)
       VALUES ($1, $2, $3, $4, 'live', CURRENT_TIMESTAMP, $5)`,
      [id, title, description || null, scripture_reference || null, req.user!.id]
    )
