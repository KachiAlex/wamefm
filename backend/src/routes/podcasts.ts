...
router.get('/', async (req, res) => {
  try {
    ...
    const rows = await db.all(
      'SELECT * FROM podcasts ORDER BY date DESC, created_at DESC'
    )
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const id = uuidv4()
    await db.run(
      `INSERT INTO podcasts (id, title, speaker, duration, audio_url, description, date)
...
router.patch('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    await db.run(
      `UPDATE podcasts SET title = $1, speaker = $2, duration = $3, audio_url = $4, description = $5, date = $6 WHERE id = $7`,
      [title ?? existing.title, speaker ?? existing.speaker, duration ?? existing.duration,
