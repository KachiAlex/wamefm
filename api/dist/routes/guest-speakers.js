...
router.get('/', async (req, res) => {
    try {
        await initDb();
        const rows = await db.all('SELECT * FROM guest_speakers WHERE is_active = TRUE ORDER BY date DESC, created_at DESC');
...
router.post('/', authenticateToken, async (req, res) => {
    try {
        ...
        const id = uuidv4();
        await db.run(`INSERT INTO guest_speakers (id, name, bio, photo_url, topic, date, is_active)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`, [id, name, bio || '', photo_url || '', topic || '', date || '', is_active !== false]);
...
router.patch('/:id', authenticateToken, async (req, res) => {
    try {
        ...
        if (!existing)
            return res.status(404).json({ error: 'Not found' });
        await db.run(`UPDATE guest_speakers SET name = $1, bio = $2, photo_url = $3, topic = $4, date = $5, is_active = $6 WHERE id = $7`, [name ?? existing.name, bio ?? existing.bio, photo_url ?? existing.photo_url,
...
//# sourceMappingURL=guest-speakers.js.map